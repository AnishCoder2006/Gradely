...
const AdminTeacherAssignPage = () => {
  ...
  const handleReject = async () => {
    ...
    try {
      await apiClient.patch(`/courses/${rejectModal.courseId}/reject`, { reason: rejectReason });
  ...
  const handleAssign = async () => {
    ...
    setAssigning(true);
    try {
      await apiClient.patch(`/courses/${selectedCourse}/assign-teacher`, { teacherId: modal.teacher._id });
  ...
  const handleUnassign = async (courseId: string, courseName: string) => {
    ...
    try {
      await apiClient.patch(`/courses/${courseId}/assign-teacher`, { teacherId: null });
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <span style={{ fontFamily: 'Geist Mono, monospace', fontSize: 11, color: 'var(--accent)', backgroundColor: 'rgba(234,179,8,0.1)', padding: '2px 8px', borderRadius: 5 }}>{course.code}</span>
                ...
                <div key={c._id} className="flex-between" style={{ padding: '12px 22px', borderBottom: i < rejectedCourses.length - 1 ? '1px solid var(--border)' : 'none', opacity: 0.65 }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontFamily: 'Geist Mono, monospace', fontSize: 10, color: 'var(--error)', backgroundColor: 'var(--error-bg)', padding: '2px 6px', borderRadius: 4 }}>{c.code}</span>
      ...
      {tab === 'assignments' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          teachers.length === 0 ? (
            ...
            const unassigned = activeCourses.filter(c => String(c.instructorId) !== teacher._id);
            return (
              <div key={teacher._id} className="card animate-fade-up" style={{ padding: 0, overflow: 'hidden', animationDelay: `${i * 40}ms` }}>
                ...
                {assigned.length === 0 ? (
                  ...
                  <div key={course._id} className="flex-between" style={{ padding: '12px 22px', borderBottom: ci < assigned.length - 1 ? '1px solid var(--border)' : 'none', transition: 'background 0.1s' }}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontFamily: 'Geist Mono, monospace', fontSize: 10, color: 'var(--accent)', backgroundColor: 'rgba(234,179,8,0.1)', padding: '2px 6px', borderRadius: 4 }}>{course.code}</span>
                      <div>
